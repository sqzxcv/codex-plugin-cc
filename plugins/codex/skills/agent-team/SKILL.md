...
Your Codex agents are senior engineers in tmux split panes. Each runs `codex --dangerously-bypass-approvals-and-sandbox` with full access to the codebase, all CLI tools, and all MCP servers. They write code, run tests, and verify their own work. You coordinate; they implement.
...
refresh attempts, so parallel expired requests race.

## What to Do
1. Add a mutex to the refresh interceptor so only one refresh runs at a time.
...
## Verification
- Run `npm test -- --grep auth` and confirm all tests pass.
...
This requires careful coordination. All agents share one working tree. No git worktrees, no branches, no isolation. If two agents edit the same file, one silently overwrites the other.
