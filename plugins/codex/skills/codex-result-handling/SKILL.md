...
- AUTOMATIC CROSS-MODEL ANALYSIS: Every time you present review or challenge findings that include CRITICAL or HIGH severity issues, you MUST automatically append a "Cross-Model Analysis" section. For each CRITICAL and HIGH finding, state whether you AGREE or DISAGREE and why in one sentence. This is not optional and does not require the user to ask for it. The whole point of running Codex is independent perspectives from a different model, not blind relay. Present both sides and let the user decide. Do this BEFORE asking which issues to fix.
