---
description: Follow up on the previous Codex session review using new Claude Code session changes
...
Run a Codex follow-up session review.
...
- It requires a previous `/codex:session-review` checkpoint for this Claude session.
- Run Codex in read-only mode through the runtime.
- The runtime focuses on transcript entries added after the previous session-review checkpoint, while including the previous Codex review and latest git state as context.
...
- Rerun Codex in read-only mode over the latest follow-up context with that supplemental input:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" session-review-follow-up "--json <effective-arguments> --user-note-file <note-file>"
```
